...
static unsigned int nSoundDelta = 0x4000;	// 33075 Hz
...
//	if(!chip->timer[0].timer || !chip->timer[1].timer) return NULL;
	...
	nSoundDelta = (unsigned int) (33075.0 * 0x4000 / nBurnSoundRate);
...
void ics2115_update(int length)
{
	...
	for(int osc=0; osc<32; osc++)
		if(chip->voice[osc].state & V_ON) {
			...
			unsigned int delta = (chip->voice[osc].fc * nSoundDelta) >> 12;

			pSoundBuf = pBurnSoundOut;
			
			for(int i=0; i<length; i++) {
				signed int v = chip->rom[badr|(adr >> 12)];
				
				if(conf & 1)v = chip->ulaw[v];
				else		v = ((signed char)v) << 8;	// ???!!!
