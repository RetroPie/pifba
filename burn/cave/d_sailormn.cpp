...
static int DrvExit()
{
	...
	if (nWhichGame)
	{
		...
		{
			UpperFree(CaveSpriteROM);
			CaveSpriteROM = NULL;
		}
		if (CaveTileROM[0] != NULL)
		{
			UpperFree(CaveTileROM[0]);
			CaveTileROM[0] = NULL;
		}
	
		if (CaveTileROM[1] != NULL)
		{
			UpperFree(CaveTileROM[1]);
			CaveTileROM[1] = NULL;
		}
	}
	else
...
static int MemIndex()
{
	...
	if (nWhichGame) {
		if (CaveTileROM[0] == NULL)
			CaveTileROM[0] = (unsigned char*)UpperMalloc(0x400000);
		if (CaveTileROM[1] == NULL)
			CaveTileROM[1] = (unsigned char*)UpperMalloc(0x400000);
		CaveTileROM[2]	= Next; Next += 0x400000;		// Tile layer 2 (agallet)
		if (CaveSpriteROM == NULL)
			CaveSpriteROM = (unsigned char*)UpperMalloc(0x800000);
	} else {
		CaveTileROM[0]	= Next; Next += 0x400000;		// Tile layer 2 (agallet)
		CaveTileROM[1]	= Next; Next += 0x400000;		// Tile layer 2 (agallet)
		if (CaveTileROM[2] == NULL)
			CaveTileROM[2]	= (unsigned char*)UpperMalloc(0x01400000);		// Tile layer 2 (Sailor Moon)
...
static int sailormnLoadRoms()
{
	...
	pTemp = (unsigned char*)UpperMalloc(0x400000);
	...
	pTemp = (unsigned char*)UpperMalloc(0x600000);
	...
	MSM6295ROM		= (unsigned char*)UpperMalloc(0x400000);		// MSM6295 ADPCM data
	// Load OKIM6295 data
...
static int agalletLoadRoms()
{
	...
	unsigned char* pTemp = (unsigned char*)UpperMalloc(0x200000);
