...
OP( 0x0f, i_pre_nec  ) { unsigned int ModRM, tmp, tmp2; /* pop cs at V30MZ? */
	switch (FETCH) {
...
OP( 0x68, i_push_d16 ) { unsigned int tmp;	FETCHWORD(tmp); PUSH(tmp);	CLK(1);	}
OP( 0x69, i_imul_d16 ) { unsigned int tmp;	DEF_r16w; 	FETCHWORD(tmp); dst = (signed int)((signed short)src)*(signed int)((signed short)tmp); I->CarryVal = I->OverVal = (((signed int)dst) >> 15 != 0) && (((signed int)dst) >> 15 != -1);     RegWord(ModRM)=(unsigned short)dst;     CLKM(4,3);}
OP( 0x6a, i_push_d8  ) { unsigned int tmp = (unsigned short)((signed short)((signed char)FETCH)); 	PUSH(tmp);	CLK(1);	}
OP( 0x6b, i_imul_d8  ) { unsigned int src2; DEF_r16w; src2= (unsigned short)((signed short)((signed char)FETCH)); dst = (signed int)((signed short)src)*(signed int)((signed short)src2); I->CarryVal = I->OverVal = (((signed int)dst) >> 15 != 0) && (((signed int)dst) >> 15 != -1); RegWord(ModRM)=(unsigned short)dst; CLKM(4,3); }
...
OP( 0x88, i_mov_br8   ) { unsigned char  src; GetModRM; src = RegByte(ModRM); 	PutRMByte(ModRM,src); 	CLKM(1,1); 			}
OP( 0x89, i_mov_wr16  ) { unsigned short src; GetModRM; src = RegWord(ModRM); 	PutRMWord(ModRM,src);	CLKM(1,1); 	}
OP( 0x8a, i_mov_r8b   ) { unsigned char  src; GetModRM; src = GetRMByte(ModRM);	RegByte(ModRM)=src;		CLKM(1,1); 		}
OP( 0x8b, i_mov_r16w  ) { unsigned short src; GetModRM; src = GetRMWord(ModRM);	RegWord(ModRM)=src; 	CLKM(1,1); 	}
OP( 0x8c, i_mov_wsreg ) { GetModRM; PutRMWord(ModRM,I->sregs[(ModRM & 0x38) >> 3]);				CLKM(1,1); }
OP( 0x8d, i_lea       ) { unsigned short ModRM = FETCH; (void)(*GetEA[ModRM])(); RegWord(ModRM)=EO; 	CLK(1); }
OP( 0x8e, i_mov_sregw ) { unsigned short src; GetModRM; src = GetRMWord(ModRM); CLKM(3,2);
    switch (ModRM & 0x38) {
...
OP( 0x8f, i_popw ) { unsigned short tmp; GetModRM; POP(tmp); PutRMWord(ModRM,tmp); CLKM(3,1); }
OP( 0x90, i_nop  ) { CLK(1);
	/* Cycle skip for idle loops (0: NOP  1:  JMP 0) */
	if (I->no_interrupt==0 && I->nec_ICount>0 && (PEEKOP((I->sregs[CS]<<4)+I->ip))==0xeb && (PEEK((I->sregs[CS]<<4)+I->ip+1))==0xfd)
		I->nec_ICount%=15;
}
...
OP( 0x9a, i_call_far  ) { unsigned int tmp, tmp2;	FETCHWORD(tmp); FETCHWORD(tmp2); PUSH(I->sregs[CS]); PUSH(I->ip); I->ip = (unsigned short)tmp; I->sregs[CS] = (unsigned short)tmp2; CLK(10); }
OP( 0x9b, i_wait      ) { ; }
OP( 0x9c, i_pushf     ) { PUSH( CompressFlags() ); CLK(2); }
OP( 0x9d, i_popf      ) { unsigned int tmp; POP(tmp); ExpandFlags(tmp); CLK(3);}
OP( 0x9e, i_sahf      ) { unsigned int tmp = (CompressFlags() & 0xff00) | (I->regs.b[AH] & 0xd5); ExpandFlags(tmp); CLK(4); }
OP( 0x9f, i_lahf      ) { I->regs.b[AH] = CompressFlags() & 0xff; CLK(2); }

OP( 0xa0, i_mov_aldisp ) { unsigned int addr; FETCHWORD(addr); I->regs.b[AL] = GetMemB(DS, addr); CLK(1); }
OP( 0xa1, i_mov_axdisp ) { unsigned int addr; FETCHWORD(addr); I->regs.b[AL] = GetMemB(DS, addr); I->regs.b[AH] = GetMemB(DS, (addr+1)&0xffff); CLK(1); }
OP( 0xa2, i_mov_dispal ) { unsigned int addr; FETCHWORD(addr); PutMemB(DS, addr, I->regs.b[AL]);  CLK(1); }
OP( 0xa3, i_mov_dispax ) { unsigned int addr; FETCHWORD(addr); PutMemB(DS, addr, I->regs.b[AL]);  PutMemB(DS, (addr+1)&0xffff, I->regs.b[AH]); CLK(1); }
OP( 0xa4, i_movsb      ) { unsigned int tmp = GetMemB(DS,I->regs.w[IX]); PutMemB(ES,I->regs.w[IY], tmp); I->regs.w[IY] += -2 * I->DF + 1; I->regs.w[IX] += -2 * I->DF + 1; CLK(5); }
OP( 0xa5, i_movsw      ) { unsigned int tmp = GetMemW(DS,I->regs.w[IX]); PutMemW(ES,I->regs.w[IY], tmp); I->regs.w[IY] += -4 * I->DF + 2; I->regs.w[IX] += -4 * I->DF + 2; CLK(5); }
OP( 0xa6, i_cmpsb      ) { unsigned int src = GetMemB(ES, I->regs.w[IY]); unsigned int dst = GetMemB(DS, I->regs.w[IX]); SUBB; I->regs.w[IY] += -2 * I->DF + 1; I->regs.w[IX] += -2 * I->DF + 1; CLK(6); }
OP( 0xa7, i_cmpsw      ) { unsigned int src = GetMemW(ES, I->regs.w[IY]); unsigned int dst = GetMemW(DS, I->regs.w[IX]); SUBW; I->regs.w[IY] += -4 * I->DF + 2; I->regs.w[IX] += -4 * I->DF + 2; CLK(6); }
...
OP( 0xae, i_scasb      ) { unsigned int src = GetMemB(ES, I->regs.w[IY]); 	unsigned int dst = I->regs.b[AL]; SUBB; I->regs.w[IY] += -2 * I->DF + 1; CLK(4);  }
OP( 0xaf, i_scasw      ) { unsigned int src = GetMemW(ES, I->regs.w[IY]); 	unsigned int dst = I->regs.w[AW]; SUBW; I->regs.w[IY] += -4 * I->DF + 2; CLK(4); }
...
OP( 0xc0, i_rotshft_bd8 ) {
	...
	if (c) switch (ModRM & 0x38) {
		case 0x00: do { ROL_BYTE;  c--; } while (c>0); PutbackRMByte(ModRM,(unsigned char)dst); break;
		case 0x08: do { ROR_BYTE;  c--; } while (c>0); PutbackRMByte(ModRM,(unsigned char)dst); break;
		case 0x10: do { ROLC_BYTE; c--; } while (c>0); PutbackRMByte(ModRM,(unsigned char)dst); break;
		case 0x18: do { RORC_BYTE; c--; } while (c>0); PutbackRMByte(ModRM,(unsigned char)dst); break;
...
OP( 0xc1, i_rotshft_wd8 ) {
	...
	CLKM(5,3);
    if (c) switch (ModRM & 0x38) {
		case 0x00: do { ROL_WORD;  c--; } while (c>0); PutbackRMWord(ModRM,(unsigned short)dst); break;
		case 0x08: do { ROR_WORD;  c--; } while (c>0); PutbackRMWord(ModRM,(unsigned short)dst); break;
		case 0x10: do { ROLC_WORD; c--; } while (c>0); PutbackRMWord(ModRM,(unsigned short)dst); break;
		case 0x18: do { RORC_WORD; c--; } while (c>0); PutbackRMWord(ModRM,(unsigned short)dst); break;
...
OP( 0xc2, i_ret_d16  ) { unsigned int count = FETCH; count += FETCH << 8; POP(I->ip); I->regs.w[SP]+=count; CLK(6); }
OP( 0xc3, i_ret      ) { POP(I->ip); CLK(6); }
OP( 0xc4, i_les_dw   ) { GetModRM; unsigned short tmp = GetRMWord(ModRM); RegWord(ModRM)=tmp; I->sregs[ES] = GetnextRMWord; CLK(6); }
OP( 0xc5, i_lds_dw   ) { GetModRM; unsigned short tmp = GetRMWord(ModRM); RegWord(ModRM)=tmp; I->sregs[DS] = GetnextRMWord; CLK(6); }
...
OP( 0xca, i_retf_d16  ) { unsigned int count = FETCH; count += FETCH << 8; POP(I->ip); POP(I->sregs[CS]); I->regs.w[SP]+=count; CLK(9); }
...
OP( 0xd4, i_aam    ) { unsigned int mult=FETCH; mult=0; I->regs.b[AH] = I->regs.b[AL] / 10; I->regs.b[AL] %= 10; SetSZPF_Word(I->regs.w[AW]); CLK(17); }
OP( 0xd5, i_aad    ) { unsigned int mult=FETCH; mult=0; I->regs.b[AL] = I->regs.b[AH] * 10 + I->regs.b[AL]; I->regs.b[AH] = 0; SetSZPF_Byte(I->regs.b[AL]); CLK(6); }
OP( 0xd6, i_setalc ) { I->regs.b[AL] = (CF)?0xff:0x00; CLK(3);  } /* nop at V30MZ? */
OP( 0xd7, i_trans  ) { unsigned int dest = (I->regs.w[BW]+I->regs.b[AL])&0xffff; I->regs.b[AL] = GetMemB(DS, dest); CLK(5); }
OP( 0xd8, i_fpo    ) { GetModRM; CLK(3);	 } /* nop at V30MZ? */

OP( 0xe0, i_loopne ) { signed char disp = (signed char)FETCH; I->regs.w[CW]--; if (!ZF && I->regs.w[CW]) { I->ip = (unsigned short)(I->ip+disp);  CLK(6); } else CLK(3); }
OP( 0xe1, i_loope  ) { signed char disp = (signed char)FETCH; I->regs.w[CW]--; if ( ZF && I->regs.w[CW]) { I->ip = (unsigned short)(I->ip+disp);  CLK(6); } else CLK(3); }
OP( 0xe2, i_loop   ) { signed char disp = (signed char)FETCH; I->regs.w[CW]--; if (I->regs.w[CW]) { I->ip = (unsigned short)(I->ip+disp);  CLK(5); } else CLK(2); }
OP( 0xe3, i_jcxz   ) { signed char disp = (signed char)FETCH; if (I->regs.w[CW] == 0) { I->ip = (unsigned short)(I->ip+disp);  CLK(4); } else CLK(1); }
OP( 0xe4, i_inal   ) { unsigned char port = FETCH; I->regs.b[AL] = read_port(port); CLK(6);	}
OP( 0xe5, i_inax   ) { unsigned char port = FETCH; I->regs.b[AL] = read_port(port); I->regs.b[AH] = read_port(port+1); CLK(6); }
OP( 0xe6, i_outal  ) { unsigned char port = FETCH; write_port(port, I->regs.b[AL]); CLK(6);	}
OP( 0xe7, i_outax  ) { unsigned char port = FETCH; write_port(port, I->regs.b[AL]); write_port(port+1, I->regs.b[AH]); CLK(6);	}

OP( 0xe8, i_call_d16 ) { unsigned int tmp; FETCHWORD(tmp); PUSH(I->ip); I->ip = (unsigned short)(I->ip+(signed short)tmp); CLK(5); }
OP( 0xe9, i_jmp_d16  ) { unsigned int tmp; FETCHWORD(tmp); I->ip = (unsigned short)(I->ip+(signed short)tmp); CLK(4); }
OP( 0xea, i_jmp_far  ) { unsigned int tmp,tmp1; FETCHWORD(tmp); FETCHWORD(tmp1); I->sregs[CS] = (unsigned short)tmp1; 	I->ip = (unsigned short)tmp; CLK(7);  }
OP( 0xeb, i_jmp_d8   ) { int tmp = (int)((signed char)FETCH); CLK(4);
	if (tmp==-2 && I->no_interrupt==0 && I->nec_ICount>0) I->nec_ICount%=12; /* cycle skip */
	I->ip = (unsigned short)(I->ip+tmp);
}
OP( 0xec, i_inaldx   ) { I->regs.b[AL] = read_port(I->regs.w[DW]); CLK(6);}
OP( 0xed, i_inaxdx   ) { unsigned int port = I->regs.w[DW];	I->regs.b[AL] = read_port(port);	I->regs.b[AH] = read_port(port+1); CLK(6); }
OP( 0xee, i_outdxal  ) { write_port(I->regs.w[DW], I->regs.b[AL]); CLK(6);	}
OP( 0xef, i_outdxax  ) { unsigned int port = I->regs.w[DW];	write_port(port, I->regs.b[AL]);	write_port(port+1, I->regs.b[AH]); CLK(6); }

OP( 0xf0, i_lock     ) {  I->no_interrupt=1; CLK(1); }
#define THROUGH 				\
	if(I->nec_ICount<0){			\
		if(I->seg_prefix)			\
			I->ip-=(unsigned short)3;	\
		else					\
			I->ip-=(unsigned short)2;	\
		break;}
...
OP( 0xf7, i_f7pre   ) { unsigned int tmp,tmp2; unsigned int uresult,uresult2; signed int result,result2;
    ...
    switch (ModRM & 0x38) {
		...
		case 0x20: uresult = I->regs.w[AW]*tmp; I->regs.w[AW]=uresult&0xffff; I->regs.w[DW]=((unsigned int)uresult)>>16; I->CarryVal=I->OverVal=(I->regs.w[DW]!=0); CLKM(4,3); break; /* MULU */
		case 0x28: result = (signed int)((signed short)I->regs.w[AW])*(signed int)((signed short)tmp); I->regs.w[AW]=result&0xffff; I->regs.w[DW]=result>>16; I->CarryVal=I->OverVal=(I->regs.w[DW]!=0); CLKM(4,3); break; /* MUL */
...
OP( 0xfe, i_fepre ) { unsigned int tmp, tmp1; GetModRM; tmp=GetRMByte(ModRM);
    switch(ModRM & 0x38) {
    	case 0x00: tmp1 = tmp+1; I->OverVal = (tmp==0x7f); SetAF(tmp1,tmp,1); SetSZPF_Byte(tmp1); PutbackRMByte(ModRM,(unsigned char)tmp1); CLKM(3,1); break; /* INC */
		case 0x08: tmp1 = tmp-1; I->OverVal = (tmp==0x80); SetAF(tmp1,tmp,1); SetSZPF_Byte(tmp1); PutbackRMByte(ModRM,(unsigned char)tmp1); CLKM(3,1); break; /* DEC */
	}
}
OP( 0xff, i_ffpre ) { unsigned int tmp, tmp1; GetModRM; tmp=GetRMWord(ModRM);
    switch(ModRM & 0x38) {
    	case 0x00: tmp1 = tmp+1; I->OverVal = (tmp==0x7fff); SetAF(tmp1,tmp,1); SetSZPF_Word(tmp1); PutbackRMWord(ModRM,(unsigned short)tmp1); CLKM(3,1); break; /* INC */
		case 0x08: tmp1 = tmp-1; I->OverVal = (tmp==0x8000); SetAF(tmp1,tmp,1); SetSZPF_Word(tmp1); PutbackRMWord(ModRM,(unsigned short)tmp1); CLKM(3,1); break; /* DEC */
		case 0x10: PUSH(I->ip);	I->ip = (unsigned short)tmp; CLKM(6,5); break; /* CALL */
